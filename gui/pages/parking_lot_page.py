...
class ParkingLotPage(ctk.CTkFrame):
    ...
    def setup_ui(self):
        ...
        # THÊM: Hiển thị vi phạm đỗ xe
        self.violation_frame = ctk.CTkFrame(self.result_frame)
        self.violation_frame.pack(fill="x", padx=20, pady=10)
        
        self.violation_title = ctk.CTkLabel(
            self.violation_frame,
            text="Vi phạm đỗ xe:",
            font=ctk.CTkFont(size=16)
        )
        self.violation_title.pack(side="left", padx=10)
        
        self.violation_value = ctk.CTkLabel(
            self.violation_frame,
            text="0 vi phạm",
            font=ctk.CTkFont(size=16, weight="bold"),
            text_color="#FF5722"
        )
        self.violation_value.pack(side="left", padx=10)
        
        # Nút điều khiển - BỎ NÚT TỰ ĐỘNG PHÁT HIỆN VẠCH VÀNG
        ...
        self.instruction_label = ctk.CTkLabel(
            self.control_frame,
            text="Hướng dẫn: Bật 'Đánh dấu vị trí' → Click chuột trái để thêm → Click chuột phải để xóa",
            font=ctk.CTkFont(size=12),
            text_color="gray70"
        )
        self.instruction_label.pack(side="left", padx=20, pady=10)
        ...
        while self.running and not self.stop_flag.is_set():
            try:
                ...
                if self.marking_mode:
                    ...
                    # DEBUG - Log thông tin vi phạm
                    if frame_count % 30 == 0:
                        violation_count = sum(1 for spot in spot_status if spot.get('is_violation', False))
                        if violation_count > 0:
                            violation_spots = [spot['spot_id'] for spot in spot_status if spot.get('is_violation', False)]
                            logger.info(f"DEBUG: {violation_count} ô vi phạm: {violation_spots}")
                    
                    # Cập nhật UI với thông tin vi phạm cụ thể
    ...
    def update_spot_status(self, spot_status, free_spots):
        """
        Cập nhật trạng thái vị trí đỗ xe - ĐÃ SỬA: Hiển thị danh sách ô vi phạm cụ thể
        """
        try:
            total_spots = len(spot_status)
            free_count = len(free_spots)
            
            # Cập nhật số vị trí trống
            self.free_spots_value.configure(text=f"{free_count} / {total_spots}")
            
            # Hiển thị danh sách vị trí trống
            if free_spots:
                free_spots.sort()
            ...
            # ĐÃ THÊM: Tìm và hiển thị các ô vi phạm cụ thể
            violation_spots = []
            for spot in spot_status:
                if spot.get('is_violation', False):  # Ô có vi phạm
                    violation_spots.append(spot['spot_id'])
            
            # Hiển thị danh sách ô vi phạm
            if violation_spots:
                violation_spots.sort()  # Sắp xếp theo thứ tự
                violation_text = f"{', '.join(violation_spots)}"
                self.violation_value.configure(
                    text=violation_text,
                    text_color="#FF5722"  # Màu đỏ cam
                )
            else:
                self.violation_value.configure(
                    text="Không có vi phạm",
                    text_color="#4CAF50"  # Màu xanh lá
                )
            
            # Log thông tin debug
            if violation_spots:
                logger.info(f"DEBUG: Phát hiện {len(violation_spots)} ô vi phạm: {violation_spots}")
    ...
    def add_status_to_frame(self, frame):
        ...
        # mode_text = "Chế độ: Đánh dấu vị trí" if self.marking_mode else "Chế độ: Giám sát"
        # cv2.putText(
        #     status_frame,
        #     mode_text, 
        #     (10, 70), 
        ...
        if self.marking_mode:
            ...
            if 0 <= real_x <= frame_w and 0 <= real_y <= frame_h:
                spot_data = {
                    ...
                    'size': (95, 250)  # Kích thước cố định
                }
